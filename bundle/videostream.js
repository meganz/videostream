 ...
 * $Id: videostream.js,v 5.6.0 2026/08/04 04:10:23 dc Exp $
 */
...
var inherits = require("/bundle/utils").inherit, Buffer = require("buffer").Buffer, stream = require("readable-stream"), EventEmitter = require("eventemitter3").EventEmitter, EBMLDecoder = require("ebml/lib/ebml/decoder"), mkv = require("./mkv-fmp4");

module.exports = EBMLRemuxer;

var MIN_FRAGMENT_DURATION = 1, DEBUG = 0 | localStorage.vsd, DEBUG_INFO = DEBUG || window.d, DEBUG_ALL = DEBUG > 3, DEBUG_TRACE = DEBUG > 2, DEBUG_VERBOSE = DEBUG > 1;
...
Fmp4Segment.prototype.appendFrames = function(frames) {
  if (!this.destroyed && frames && frames.length) {
    ...
    for (var limit = MIN_FRAGMENT_DURATION * track.timescale, from = 0, i = 0; i < frames.length; i++) if (frames[i].keyframe && i > from && frames[i].dts - frames[from].dts >= limit) {
      this.push(mkv.buildMediaSegment(track, frames.slice(from, i)));
      from = i;
    }
    from < frames.length && this.push(mkv.buildMediaSegment(track, frames.slice(from)));
  }
};
...
},{"./mkv-fmp4":6,"/bundle/utils":4,"buffer":11,"ebml/lib/ebml/decoder":12,"eventemitter3":16,"readable-stream":42}],6:[function(require,module,exports){
"use strict";
...
function assignTimestamps(samples, defaultDuration) {
  for (var reordered = !1, i = 1; i < samples.length; i++) if (samples[i].pts < samples[i - 1].pts) {
    reordered = !0;
    break;
  }
  if (reordered) for (var sorted = samples.map(function(s) {
    return s.pts;
  }).sort(function(a, b) {
    return a - b;
  ...
  } else for (i = 0; i < samples.length; i++) {
    samples[i].dts = samples[i].pts;
    samples[i].cts = 0;
  }
  for (i = 0; i < samples.length; i++) {
    samples[i].duration = i + 1 < samples.length ? samples[i + 1].dts - samples[i].dts : defaultDuration;
    samples[i].duration > 0 || (samples[i].duration = defaultDuration);
  }
}
